...
for c in class_names:
 ...
 for f in range(len(features)):
    ...
    prob = (count_x_c + 1) / (count_c + 3) #laplace smoothing
    l_probs += np.log(prob)
...
def softmax(arr, not_arr):
  ...
  return [prob_arr, prob_not_arr] #returns probs as a 1D list
scores = softmax(class_scores['spam'], class_scores['not_spam'])

#print our probs for our spam filter model(2 d.p js like in A level physics)
